...
for mall in mall_list:
	...
	#新加规则，留下满足要求的wifi
	include_wifi = []
	with open(path + 'dictionary/' + mall + '_wifi.pkl','rb') as f:
		sidKey = pickle.load(f)
		for sid in sidKey:
			wifiSort = sorted(sidKey[sid].items(), key=operator.itemgetter(1), reverse=True)[:max(10, len(sidKey[sid]) // 3)]
			for i in wifiSort:
				include_wifi.append(i[0])

	#数据copy，只添加在include_wifi中记录的wifi
	m = []
	for row in l:
		new = {}
		for n in row.keys():
			if n in include_wifi:
				new[n] = row[n]
		m.append(new)
	...
	result.to_csv(path + 'xgbsklearn_ruletop_85.csv', index = False)
	print("End modelfit")

report = pd.DataFrame.from_records([df_report])
report.to_csv(path + 'xgbsklearn_ruletop_85_train.csv', index = False)
