...
#		print(sidSum)
		...
		for key in sidKey:
			for num in range(len(wifiInfoSort)):
				if wifiInfoSort[num][0] in sidKey[key]:
					if not wifiInfoSort[num][0] in wifiInSid:
						wifiInSid[wifiInfoSort[num][0]] = 1
					else:
						wifiInSid[wifiInfoSort[num][0]] += 1
#		print(wifiInSid)
...
#								predict[rid][key] = sidKey[key][wifiInfoSort[num][0]][2] / abs(int(wifiInfoSort[num][1]) - sidKey[key][wifiInfoSort[num][0]][1]) / (num + 1)
							predict[rid][key] = sidKey[key][wifiInfoSort[num][0]][2] * math.log(sidSum/(wifiInSid[wifiInfoSort[num][0]]+1)) / (num + 1)**2
						else:
#								predict[rid][key] += sidKey[key][wifiInfoSort[num][0]][2] / abs(int(wifiInfoSort[num][1]) - sidKey[key][wifiInfoSort[num][0]][1]) / (num + 1)
							predict[rid][key] += sidKey[key][wifiInfoSort[num][0]][2] * math.log(sidSum/(wifiInSid[wifiInfoSort[num][0]]+1)) / (num + 1)**2
				except:
